...
select concat_ws(' ', ap_paterno, ap_materno, a.nombre) alumno, avg(calificacion) promedio, cu.abreviatura curso, cu.id_curso
from alumnos a
join evaluaciones e on (a.clave_alu = e.clave_alu)
join cursos cu on (e.id_curso = cu.id_curso)
where abreviatura = '2018-2019'
group by calificacion, cu.abreviatura, cu.id_curso, concat_ws(' ', ap_paterno, ap_materno, a.nombre) 
order by alumno;
...
select concat_ws(' ', ap_paterno, ap_materno, a.nombre) alumno, avg(pago) prom_pago, c.abreviatura curso
from alumnos a
...
select concat_ws(' ', apellido_p, apellido_m, nombres) profesor, p.clave_prof 
from profesores p
join profesor_grupo ps on (p.clave_prof = ps.clave_prof);